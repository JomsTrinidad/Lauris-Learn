-- Migration 025: Fix infinite recursion in parent portal RLS policies
--
-- Root cause (migration 024): policies on students/enrollments/classes/etc. used
-- EXISTS (SELECT 1 FROM guardians ...). The existing "School members can view guardians"
-- policy does student_id IN (SELECT id FROM students ...), which re-triggers the
-- students policy that queries guardians → infinite loop.
--
-- Fix: a SECURITY DEFINER function reads guardians without RLS applied, breaking
-- the cycle. All parent policies use this function instead of querying guardians directly.

-- ── Helper function ───────────────────────────────────────────────────────────────
-- Returns student_ids for the currently authenticated user's email, bypassing RLS.
...
-- ── Drop the recursive policies added in 024 ─────────────────────────────────────
...
-- ── Recreate policies using parent_student_ids() ─────────────────────────────────
...
CREATE POLICY "parents_select_class_updates"
  ...
  USING (
    class_id IS NULL  -- school-wide broadcasts visible to all parents
    OR class_id IN (
      SELECT e.class_id FROM enrollments e
      WHERE e.student_id = ANY(parent_student_ids())
    )
