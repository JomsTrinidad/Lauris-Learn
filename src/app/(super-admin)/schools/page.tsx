...
export default function SuperAdminSchoolsPage() {
  ...
  async function handleCreate() {
    ...
    const { data: newSchool, error: schoolErr } = await supabase
      ...
      .insert({
        name: form.name.trim(),
        trial_start_date: form.trialStartDate || null,
        trial_end_date: form.trialEndDate || null,
        trial_status: form.trialStatus,
      })
      .select("id")
      .single();

    if (schoolErr) { setFormError(schoolErr.message); setSaving(false); return; }

    // Seed a default school year for the new school
    ...
    // If admin email provided, update their profile
    if (form.adminEmail.trim()) {
      await supabase
        ...
        .update({
          school_id: newSchool.id,
          role: "school_admin",
          full_name: form.adminName.trim() || form.adminEmail,
        })
        .eq("email", form.adminEmail.trim().toLowerCase());
    }
  ...
  function handleImpersonate(school: School) {
    // Store impersonation data and navigate to dashboard
