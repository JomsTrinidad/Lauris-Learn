...
import {
  ...
  HelpCircle, X, Search, ChevronRight, Key, School as SchoolIcon, Copy,
} from "lucide-react";
...
function CredentialsPanel({ creds }: { creds: DemoCredentials }) {
  const [open, setOpen] = useState(false);
  const allRows = [
    { role: "School Admin", email: creds.admin.email, name: creds.admin.fullName },
    ...creds.teachers.map((t) => ({ role: "Teacher", email: t.email, name: t.fullName })),
    ...creds.parents.map((p) => ({ role: "Parent", email: p.email, name: p.fullName })),
  ];
  const allText = [
    `Password (all accounts): ${creds.password}`,
    "",
    ...allRows.map((r) => `${r.role.padEnd(14)} ${r.email}  (${r.name})`),
  ].join("\n");
    ...
    <div className="border-t border-border bg-muted/20">
      ...
      >
        <span className="flex items-center gap-1.5 text-muted-foreground">
          ...
          Demo logins <span className="text-foreground font-medium">({allRows.length})</span>
          <span className="text-[10px] text-muted-foreground/70 ml-1">— stable across refreshes</span>
        </span>
      ...
      {open && (
        <div className="px-4 pb-3 space-y-3">
          <div className="flex items-center justify-between gap-3 p-2.5 rounded-lg bg-card border border-border">
            <div>
              <p className="text-[10px] uppercase tracking-wide text-muted-foreground">Password (all accounts)</p>
              <p className="text-sm font-mono text-foreground">{creds.password}</p>
            </div>
            <div className="flex items-center gap-3">
              <CopyButton text={creds.password} label="password" />
              <CopyButton text={allText} label="full credential list" />
            </div>
          </div>
              ...
              <tbody>
                {allRows.map((r) => (
                  <tr key={r.email} className="border-t border-border">
                    ...
                    <td className="px-3 py-1.5 text-right"><CopyButton text={r.email} label={r.email} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {demoSchools.map((school) => {
          ...
          return (
            <Card key={school.id} className="overflow-hidden">
              ...
              {/* Demo credentials (stable across refreshes) */}
              {summary?.credentials && (
                <CredentialsPanel creds={summary.credentials} />
              )}
              ...
              {(() => {
                ...
                const topics: HelpTopic[] = [
                  ...
                  },
                  {
                    ...
                    searchText: "credentials login password email teacher parent admin school stable share clients accounts demo users",
                    body: (
                      <div className="space-y-2">
                        <p>Demo logins are <strong className="text-foreground">stable across refreshes</strong> — share them once with a client and they keep working after every refresh.</p>
                        <p>Emails and the password are derived from the school&apos;s name (first word, lowercased). For a school named <em>Sunshine Learning Center</em> the slug is <code className="bg-muted px-1 rounded text-xs">sunshine</code>.</p>
                        <div className="space-y-1.5">
                          <div className="bg-muted/50 rounded-lg p-2.5 border border-border">
                            <p className="text-xs font-semibold text-foreground mb-1">School Admin (1 per school)</p>
                            <p className="text-xs font-mono">admin@{"{slug}"}.test</p>
                          </div>
                          <div className="bg-muted/50 rounded-lg p-2.5 border border-border">
                            <p className="text-xs font-semibold text-foreground mb-1">Teachers</p>
                            <p className="text-xs font-mono">t1@{"{slug}"}.test, t2@{"{slug}"}.test, …</p>
                          </div>
                          <div className="bg-muted/50 rounded-lg p-2.5 border border-border">
                            <p className="text-xs font-semibold text-foreground mb-1">Parents</p>
                            <p className="text-xs font-mono">p1@{"{slug}"}.test, p2@{"{slug}"}.test, …</p>
                          </div>
                          <div className="bg-muted/50 rounded-lg p-2.5 border border-border">
                            <p className="text-xs font-semibold text-foreground mb-1">Password (all accounts)</p>
                            <p className="text-xs font-mono">{"{Slug}"}Demo2026!  (e.g. SunshineDemo2026!)</p>
                          </div>
                        </div>
                        <Note>The full list — with a one-click copy — appears in the <strong>Demo logins</strong> drawer at the bottom of each school card after a successful generate or refresh.</Note>
                        <Tip>Two demo schools whose names start with the same first word would collide on email. Rename one if that ever happens.</Tip>
                      </div>
                    ),
                  },
                  {
